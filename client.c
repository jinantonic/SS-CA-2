...
int main() { // Main function
	...
	while ((client_sock = accept(socket_des, (struct sockaddr *) &client, (socklen_t *) &con_size))) { // While accepting connections
		...
		pthread_join(client_conn[thread_count], NULL); // Join threads
...
void *connection_handler(void *socket_des) // Thread handler
{
	...
	READSIZE = recv(sock, &client_user_id, sizeof(client_user_id), 0); // Receive client user ID
    ...
    printf("Client id: %d\n", client_user_id); // Print client user ID
    ...
    printf("Client id: %s\n", user_name); // Print client user name

	int ngroups = 20; // Number of groups
	groups = malloc(ngroups * sizeof(gid_t)); // Allocate memory for groups

	if (getgrouplist(user_name, client_user_id, groups, &ngroups) == -1) { // If getgrouplist fails
	...
	READSIZE = recv(sock, message, 500, 0); // Receive message
    ...
    if (!user_in_group(group_id, groups, ngroups)) {
	...
	if (fr == NULL) { // If file does not exist
